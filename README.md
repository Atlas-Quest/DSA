...
| Prime Check | `is_prime.ts`   | O(n)            | O(1)             |
...
├── is_prime.ts         # Trial-division primality check
...
- **Prime Check**: `isPrime` returns `false` for values less than 2, then checks divisors from 2 to `n - 1` via trial division. Returns `true` if no divisor is found.
